...
문서 지향 DB:
    1. 특성: JSON, BSON 등의 형식으로 문서를 저장
    2. 유연성: 스키마가 고정되어 있지 않음
    3. 확장성: 수평적 확장
키-값 저장소:
    1. 특성: 간단한 키-값 쌍으로 데이터를 저장
    2. 유연성: 단순하고 빠른 데이터 액세스를 제공
    3. 확장성: 매우 높은 확장성
칼럼 지향 DB:
    1. 특성: 행보다는 열 중심으로 데이터를 저장
    2. 유연성: 매우 큰 데이터를 효율적으로 저장하고 조회할 수 있음
    3. 확장성: 대규모 데이터를 처리하는 데 적합한 수평적 확장성을 제공
그래프 DB:
    1. 특성: 노드와 엣지를 사용하여 데이터를 저장
    2. 유연성: 복잡한 관계형 데이터를 표현하고 탐색
    3. 확장성: 그래프 구조의 데이터 처리에 최적화
**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**
유연성: 몽고DB는 스키마가 동적이고 유연하며, RDBMS는 고정된 스키마를 가짐
확장성: 몽고DB는 수평적 확장, RDBMS는 수직적 확장
데이터 모델: 몽고DB는 문서 기반, RDBMS는 테이블 기반
