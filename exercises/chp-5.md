...
6. WHERE절의 LIKE 'g고’는 열 값으로 god, good, gd 등을 갖는 행들을 검색한다. (__x_)
...
2. SQL은 대화식 그리고 (프로그램 내장)의 2가지 방식으로 사용할 수 있다.
...
11. 중첩 질의문 중에서 내부 질의의 WHERE절 검색조건식이 외부 질의에 선언된 테이블의 일부 열을 참조하는 질의를 (서브쿼리)이라고 한다.
...
과목(과목번호, 과목이름, 학점, 학과, 담당교수)
`
3. 각 학생들의 학반 과목번호, 수강과목별 총점(중간성적과 기말성적의 합)을 검색하시오.
    `SELECT 학번, 과목번호, (중간성적 + 기말성적) AS 총점
FROM 수강;
`
5. 각 학생들의 이름, 수강한 모든 과목의 총점(중간성적과 기말성적의 합)을 검색하시오.
   `SELECT 학생.이름, SUM(중간성적 + 기말성적) AS 총점
FROM 학생
JOIN 수강 ON 학생.학번 = 수강.학번
GROUP BY 학생.이름;
`
7. 각 과목별 기말평균이 90점 이상인 과목의 번호와 과목별 기말성적의 평균점수를 검색하시오.
   `SELECT 과목번호, AVG(기말성적) AS 평균기말성적
FROM 수강
GROUP BY 과목번호
HAVING AVG(기말성적) >= 90;
`
9. 각 평가학점별로 취득한 학생의 수를 검색하시오.
    `SELECT 평가학점, COUNT(*) AS 학생수
FROM 수강
GROUP BY 평가학점;
`
11. ‘이씨 성을 가진 모든 학생들의 이름과 과목이름 평가학점을 검색하시오.
...
WHERE 학생.이름 LIKE '이%';
`
13. 컴퓨터학과에 소속된 학생들의 이름과 수강한 과목이름을 검색하시오.
