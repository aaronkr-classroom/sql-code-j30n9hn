...
장점: 검색속도 향상, 성능 최적화, 정렬된 데이터 접근
단점: 저장공간, 쓰기 성는 저하, 복잡성 증가
**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**
바람직한 경우: 조회 작업이 빈번할 때, 대량의 데이터에서 검색할 때, 정렬된 데이터가 자주 팔요할 때
바람직하지 않은 경우: 쓰기 작업이 빈번할 때, 소규모 데이터셋, 자주 변경되는 컬럼럼
